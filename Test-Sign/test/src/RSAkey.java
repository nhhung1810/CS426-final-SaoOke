...
class RSAKey {
    ...
    private static KeyPair parseKey(String publicPath, String privatePath) throws Exception {
        String privateKeyContent = new String(
                Files.readAllBytes(Paths.get(ClassLoader.getSystemResource(privatePath).toURI())));
        String publicKeyContent = new String(
                Files.readAllBytes(Paths.get(ClassLoader.getSystemResource(publicPath).toURI())));
    ...
    // only for testing
    public static KeyPair parseKey()
